...
const std::string UNIDEN_MARKER_KEY = "unidentified_markers";
...
void processMocapData( const char** mocap_model, RigidBodyMap& published_rigid_bodies, MarkerMap& published_markers)
{
        ...
        if (header == NAT_FRAMEOFDATA && version)
        {
          ...
          if( format.model.numRigidBodies > 0 )
          {
            for( int i = 0; i < format.model.numRigidBodies; i++ )
            {
              ...
              {
                  item->second.publish(format.model.rigidBodies[i], format.model.markerSets[i]);
		  //ROS_INFO("%s...%d", format.model.markerSets[i].name, i);
              }
            }
          }
          ...
          if( format.model.numOtherMarkers > 0 )
          {
            ...
            {
              //int ID = format.model.rigidBodies[i].ID;
              MarkerMap::iterator item = published_markers.find(i);

              if (item != published_markers.end())
              {
                  item->second.publish(format.model.otherMarkers[i]);
              }
            }
          }
        }
...
{ 
  ...
  MarkerMap published_markers;
    ...
    if (n.hasParam(UNIDEN_MARKER_KEY))
  {
      XmlRpc::XmlRpcValue marker_list;
      n.getParam("unidentified_markers", marker_list);
      if (marker_list.getType() == XmlRpc::XmlRpcValue::TypeStruct && marker_list.size() > 0)
      {   
          XmlRpc::XmlRpcValue::iterator i;
          for (i = marker_list.begin(); i != marker_list.end(); ++i) {
              if (i->second.getType() == XmlRpc::XmlRpcValue::TypeStruct) {
                  PublishedMarker point(i->second);
                  string id = (string&) (i->first);
                  MarkerItem item(atoi(id.c_str()), point);

                  std::pair<MarkerMap::iterator, bool> result = published_markers.insert(item);
                  if (!result.second)
                  {
                      ROS_ERROR("Could not insert configuration for unidentified_marker ID %s", id.c_str());
                  }
//          	  ROS_INFO("Markers in yaml file is successfully registered...\n");
              }
          }
      }
  }
  ...
  processMocapData(mocap_model, published_rigid_bodies, published_markers);
