...
// Tracking unidentified markers by Yan

bool Marker::has_data()
{
    static const char zero[sizeof(positionX)] = { 0 };
    return memcmp(zero, (char*) &positionX, sizeof(positionX));
}

const geometry_msgs::PointStamped Marker::get_ros_pose()
{
  geometry_msgs::PointStamped ros_point;
  ros_point.header.stamp = ros::Time::now();
  // y & z axes are swapped in the Optitrack coordinate system
  ros_point.point.x = -positionX;
  ros_point.point.y = positionZ;
  ros_point.point.z = positionY;
