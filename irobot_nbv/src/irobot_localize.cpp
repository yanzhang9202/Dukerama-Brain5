...
{
  ros::init(argc, argv, "irobot_localize");
  ros::NodeHandle n;
  ...
  // MAIN LOOP
  while( ros::ok() && (counter_meas < NUM_MEASUREMENT) ) // Check if all waypoints are sent.
  {
    ...
    count++;
    counter_meas++;
    std::cout << "The " << counter_meas << "th measurement has been done!" << std::endl << std::endl;
