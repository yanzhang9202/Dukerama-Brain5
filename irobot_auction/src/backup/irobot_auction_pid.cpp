...
//      ocam_hand << -0.06156, -0.02345, 0.00723;
      ...
      Rot_c2h <<  0.9919,  0.1141, -0.0557,
                  0.0538,  0.0200,  0.9984,
                  0.1151, -0.9933,  0.0137; 
      ocam_hand << 0.0025, 0.0857, 0.0074;

      std::cout << "Using rectified image calibration results..." << "\n" << std::endl; 
  }

// Servo_Base calibration is done on Sep 26th. Redefine robots need recalibration.
//  Rot_r2n << -0.2483, -0.9686, 0,
//	      0.9681, -0.2486, 0,
//    	      0,       0,      1;

// Servo_Base calibration is done on Oct 14th. Redefine robots need recalibration.
  Rot_r2n <<  0.5726, -0.8197, 0,
	      0.8197,  0.5726, 0,
    	      0,       0,      1;
...
//        std::cout << waypoint(0) << " " << waypoint(1) << " " << waypoint(2) << std::endl;
        ifClosetoWpt();
        if (flag_i_turned)
        { base_pid.setParam(0.5, 0.2, 0.1, 0.5, -0.5, PI/180); }
        adjustBase();
//        ifExceedServoLimit();
        if (flag_i_turned)
        {
          base_pid.setParam(0.5, 0.2, 0.1, 0.4, -0.4 , Base_Orientation_Error_Tolerance/180*PI);
          flag_i_turned = false;
        }
        adjustBase();
//        ifExceedServoLimit();
